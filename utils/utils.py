...
def log_losses_wandb(epoch,  bach_idx, num_batches, log_dict, batch_size, weights):
    ...
    for loss_term_name in weights.keys():
        if loss_term_name+"_term" in log_dict:
            log_dict["weighted/"+loss_term_name+"_term"] = weights[loss_term_name] * log_dict[loss_term_name+"_term"].detach().cpu().numpy()
    wandb.log(log_dict)
    return step
...
def compute_deriv_props_moe(decoder, INR, mnfld_points, latent, z_gt, device):

    # compute derivative properties on a grid
    res = z_gt.shape[1]
    x, y, grid_points = get_2d_grid_uniform(resolution=res, range=1.0, device=device)
    grid_points = grid_points.unsqueeze(0)
    grid_points.requires_grad_()

    out = INR(grid_points, mnfld_points)
    # grid_points_rep = out["non_mnfld_pnts_rep"]
    z_all = out["nonmanifold_pnts_pred"]
    nonmnfld_q = out["nonmnfld_q"]
    selected_expert_idx = out["nonmnfld_selected_expert_idx"]
    selected_expert_idx_np = selected_expert_idx.detach().cpu().numpy().reshape(x.shape[0], x.shape[0])

    # compute derivatives
    # grid_points_rep.requires_grad_()
    grid_points.requires_grad_()
    grid_grad = experts_gradient(grid_points, z_all)
    q_grad = experts_gradient(grid_points, nonmnfld_q).norm(2, dim=-1)
    q_grad = q_grad.gather(dim=1, index=selected_expert_idx[None, None, :]).cpu().detach().numpy().reshape(x.shape[0], x.shape[0])
    # q_grad = q_grad.max(dim=1)[0].cpu().detach().numpy().reshape(x.shape[0], x.shape[0])
    # dx = gradient(grid_points_rep, grid_grad[:, :, :, 0], create_graph=False, retain_graph=True)
    # dy = gradient(grid_points_rep, grid_grad[:, :, :, 1], create_graph=False, retain_graph=False)
    # dx = dx.gather(dim=0, index=selected_expert_idx[None, None, :, None].repeat(1, 1, 1, dx.shape[-1])).squeeze()
    # dy = dy.gather(dim=0, index=selected_expert_idx[None, None, :,  None].repeat(1, 1, 1, dy.shape[-1])).squeeze()
    # grid_curl = (dx[:, 1] - dy[:, 0]).cpu().detach().numpy().reshape(x.shape[0], x.shape[0])
    grid_curl = None
    grid_grad = grid_grad.gather(dim=1, index=selected_expert_idx[None, None, :, None].repeat(1, 1, 1, grid_grad.shape[-1]))
    ...
    #plot z difference image
    z = out["selected_nonmanifold_pnts_pred"]
    z_all = out['nonmanifold_pnts_pred']
    z_np = z.detach().cpu().numpy().reshape(x.shape[0], x.shape[0])
    z_all_np = z_all.detach().cpu().numpy().reshape(z_all.shape[1], x.shape[0], x.shape[0])
    z_diff = np.abs(np.abs(z_np) - np.abs(z_gt))

    #compute regmoesdf
    mu = (z_all * nonmnfld_q / nonmnfld_q.sum(1, keepdim=True)).sum(1, keepdim=True)
    var_regmoesdf = ((z_all - mu) ** 2 * nonmnfld_q / nonmnfld_q.sum(1, keepdim=True)).sum(1).cpu().detach().numpy().reshape(x.shape[0], x.shape[0])

    nonmnfld_q = nonmnfld_q.detach().cpu().numpy().reshape(-1, x.shape[0], x.shape[0])
    return x, y, z_np, z_diff, eikonal_term, grid_div, grid_curl, selected_expert_idx_np, nonmnfld_q, var_regmoesdf, q_grad, z_all_np
...
def get_2d_grid_uniform(resolution=100, range=1.2, device=None):
    ...
    grid_points = get_cuda_ifavailable(torch.tensor(np.stack([xx, yy], axis=1).astype('f')),
                                             device=device)
    return x, grid_points
...
def implicit2mesh(decoder, latent, grid_res, translate=[0., 0., 0.], scale=1, get_mesh=True, device=None,
    ...
    for point in tqdm(torch.split(pnts, 100000, dim=0)):
        ...
        point = get_cuda_ifavailable(point, device=device).unsqueeze(0)
        out = decoder(point.type(torch.float32))
        if 'selected_nonmanifold_pnts_pred' in out:
            z.append(out['selected_nonmanifold_pnts_pred'].squeeze().detach().cpu().numpy())
        else:
            z.append(out['nonmanifold_pnts_pred'].squeeze().detach().cpu().numpy())
    ...
    if 'selected_nonmanifold_pnts_pred' in out:
        # https://sashamaps.net/docs/resources/20-colors/ , convienent ordering
        # Red, Green, Yellow, Blue,, Orange, Purple, Cyan, Magenta, Lime, Pink, ...
        distinct_colour_list = [(230, 25, 75), (60, 180, 75), (255, 225, 25), (0, 130, 200), (245, 130, 48), (145, 30, 180), 
        ...
        distinct_colour_list = get_cuda_ifavailable(torch.tensor(np.array(distinct_colour_list, dtype=np.int32)))
        vpoints = get_cuda_ifavailable(torch.tensor(verts, dtype=torch.float32), device=device).unsqueeze(0)
        out = decoder(vpoints.type(torch.float32))
        vertex_colours = distinct_colour_list[out['nonmnfld_selected_expert_idx']].cpu().numpy()
    else:
        vertex_colours = values

    verts = verts * (1/scale) - translate

    if get_mesh:
        mesh = trimesh.Trimesh(verts, faces, vertex_normals=normals, vertex_colors=vertex_colours, validate=True)
    trace = None
...
def implicit2mesh_moe(moe_model, grid_res, translate=[0., 0., 0.], scale=1, get_mesh=True, device=None,
                  bbox=np.array([[-1, 1], [-1, 1], [-1, 1]])):
    # compute a mesh from the implicit representation in the decoder.
    # Uses marching cubes.
    # reimplemented from SAL get surface trace function : https://github.com/matanatz/SAL/blob/master/code/utils/plots.py
    ...
    for point in tqdm(torch.split(pnts, 100000, dim=0)):
        ...
        out_dict = moe_model(point.unsqueeze(0).type(torch.float32))
        selected_z = out_dict["selected_nonmanifold_pnts_pred"]
        z.append(selected_z.squeeze().detach().cpu().numpy())
    z = np.concatenate(z, axis=0).reshape(grid_dict['xyz'][1].shape[0], grid_dict['xyz'][0].shape[0],
                                          grid_dict['xyz'][2].shape[0]).transpose([1, 0, 2]).astype(np.float64)
    # import pdb; pdb.set_trace()
    print(z.min(), z.max())
...
def get_pre_post_activation_gardients(x, preactivations, postactivations):
    pre_grads = []
    post_grads = []
    for i in range(len(preactivations)):
        pre_grads.append(grad(preactivations[i], x, grad_outputs=torch.ones_like(preactivations[i]), retain_graph=True)[0])
    for i in range(len(postactivations)):
        post_grads.append(grad(postactivations[i], preactivations[i], grad_outputs=torch.ones_like(postactivations[i]), retain_graph=True)[0])
    return pre_grads, post_grads
